...
const setBorder = (ctx, borderParams, block) => {
    const { x, y } = block;
    const w = block.width;
    const h = block.height;
    if (borderParams.width)
        ctx.lineWidth = borderParams.width;
    if (borderParams.color)
        ctx.strokeStyle = borderParams.color;
    const p = borderParams.width / 2;
...
export { App, BaseNode, Block, Layer, Sprite };
